...
# OLED Deep Black & Psychedelic Purple Styling
st.markdown("""
    <style>
    .stApp {
...
st.title("Spyder")
st.caption("Quiet, ruthless productivity.")
...
if not elevenlabs_key or not supabase_url or not supabase_key:
    st.error("Missing credentials in environment secrets.")
    st.stop()
...
def upload_to_supabase_storage(file_bytes: bytes, filename: str) -> str:
    """Uploads MP3 bytes to Supabase Storage bucket and returns the public URL."""
...
def log_audiobook_to_db(title: str, audio_url: str, file_size: int):
    """Inserts metadata record into the audiobooks table."""
...
if uploaded_file is not None:
    st.write(f"Attached: **{uploaded_file.name}** ({round(uploaded_file.size / 1024, 1)} KB)")
    ...
    if st.button("Synthesize"):
        with st.status("Processing...", expanded=True) as status:
            ...
            st.write("Generating voice stream...")
